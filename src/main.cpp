...
#if defined(NDEBUG)
# error "Glastocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6b82c75cf24e6beb75f3c70703d15ae55b89b00fc7bda616f4217d0c46e8d1b2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Glastocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Glastocoin Signed Message:\n";
...
{
    // Glastocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Glastocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Glastocoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Glastocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Glastocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Glastocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        pchMessageStart[0] = 0xa1; //change last 2 digits on  first two
        pchMessageStart[1] = 0xd3;
        pchMessageStart[2] = 0xb7;
        pchMessageStart[3] = 0xdc;
        hashGenesisBlock = uint256("0x6b82c75cf24e6beb75f3c70703d15ae55b89b00fc7bda616f4217d0c46e8d1b2");  //new genesis block here
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].nValue = 50 * COIN;  //  change the first 5 digits on pubkey
        ...
        block.nVersion = 1;
        block.nTime    = 1459438514;  //google lastest epoch time
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 7143593;  //put new nonce here

        if (fTestNet)
        {
            block.nTime    = 1459438514;  //and again
            block.nNonce   = 7143593;  //and again 
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x3bf757bdc52c85a20cf6ac0c46240460fb3bade6590844a2e63f11897fb90b20"));  //delete code below after genesis block made
if (true && block.GetHash() != hashGenesisBlock)
        {
            ...
            // delete to here for genesis hash creation
            }
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Glastocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GlastocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GlastocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GlastocoinMiner(CWallet *pwallet)
{
    printf("GlastocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("glastocoin-miner");
    ...
    try { loop {
        ...
        printf("Running GlastocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GlastocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GlastocoinMiner, pwallet));
}
